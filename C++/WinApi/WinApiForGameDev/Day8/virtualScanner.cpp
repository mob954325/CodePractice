...
void scanVirtualPages(HANDLE handleToProcess)
{
	...
	while (true)
	{
		// 지정된 프로세스의 가상 주소 공간 내 페이지 범위에 대한 정보 찾기
		SIZE_T readBytes =
			VirtualQueryEx(handleToProcess, currentScanAddress, &memInfo, sizeof(_MEMORY_BASIC_INFORMATION));
		...
		{
			// this is a read write page, don't forget that there are other flags

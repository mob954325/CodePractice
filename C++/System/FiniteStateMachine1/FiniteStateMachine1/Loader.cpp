...
void from_json(const nlohmann::json& j, FrameInfo& frameinfo)
{
	...
	frameinfo.sprite = temp;
	j.at("time").get_to(frameinfo.duration);
}

void AnimationClipLoader::LoadAnimationClip(const std::string& filePath, AnimationClip& clip, const SpriteSheet& spriteSheet)
{
	...
	if (inFile.is_open()) {
		...
		for (auto& frameIndex : clip.frames) {
			...
			if (it != spriteSheet.spriteIndexMap.end()) {
				...
				throw std::runtime_error("Sprite not found in sprite sheet: " + frameIndex.sprite);
			}
		}
	}
	else {
...
void AnimationControllerLoader::LoadAnimatorController(const std::string& filePath, AnimatorController& out)
{
	std::ifstream inFile(filePath);
	if (inFile.is_open())
	{

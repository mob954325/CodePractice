#include <stdio.h>
#include <random>

// 게임 규칙
// 1. 랜덤한 숫자 3개를 생성하고 플레이어는 숫자 3개를 마춘다.
// 2. 6회 시도안에 맞추면 축하 메세지 출력
// 3. 맞추기 실패마다 Ball(자리x, 숫자o), Strike(자리o, 숫자o), Out(자리x, 숫자x)의 수를 알려준다.
// 4. 최대 6회까지 맞추지 못하면 랜덤 숫자를 알려주고 종료한다.
 
int MAX_BALLVALUE = 11;
int MAXSIZE = 3;
void SetNum(int balls[])
{
	int used[3] = { 0,0 };

	srand(time(NULL)); // 의사 번호 생성기 초기값 설정

	int randomValue = rand();
	int valueInMax = randomValue % MAX_BALLVALUE;

	int count = 0;
	balls[count] = valueInMax;
	used[count] = valueInMax;
	count++;

	while (count < MAXSIZE)
	{
		int randomValue = rand();
		int valueInMax = randomValue % MAX_BALLVALUE;

		for (int i = 0; i < MAXSIZE - 1; i++)
		{
			if (valueInMax == used[i]) continue; // 중복 숫자
		}

		// 중복숫자 아님
		balls[count] = valueInMax;
		used[count] = valueInMax;
		count++;
	}
}

int CheckNum(int arr[], int num)
{
	for (int i = 0; i < MAXSIZE; i++)
	{
		if (arr[i] == num) return i; // 인덱스 반환
	}

	return -1;
}

int main()
{
	// 초기화
	int count = 0;
	int MAX_COUNT = 6;

	// 랜덤 숫자 생성
	int balls[3] = { 0 ,0, 0 };
	SetNum(balls);

	for (int i = 0; i < MAXSIZE; i++)
	{
		printf("%d ", *(balls + i));
	}
	printf("\n");

	int v1 = 0, v2 = 0, v3 = 0;
	int outCount = 0, strikeCount = 0, ballCount = 0;
	while (count < MAX_COUNT)
	{
		scanf_s("%d %d %d", &v1, &v2, &v3);

		// 확인
		int p1 = CheckNum(balls, v1);
		int p2 = CheckNum(balls, v2);
		int p3 = CheckNum(balls, v3);

		if (p1 == -1)
		{
			outCount++;
		}
		else
		{
			if (p1 == 0)
			{
				strikeCount++;
			}
			else
			{
				ballCount++;
			}
		}

		if (p2 == -1)
		{
			outCount++;
		}
		else
		{
			if (p2 == 1)
			{
				strikeCount++;
			}
			else
			{
				ballCount++;
			}
		}

		if (p3 == -1)
		{
			outCount++;
		}
		else
		{
			if (p3 == 2)
			{
				strikeCount++;
			}
			else
			{
				ballCount++;
			}
		}

		printf("ball : %d\n", ballCount);
		printf("strike : %d\n", strikeCount);
		printf("out : %d\n", outCount);


		if (strikeCount == 3)
		{
			printf("You Win !!!\n\n");
			printf("Count : %d\n", count);
			break;
		}

		ballCount = 0;
		strikeCount = 0;
		outCount = 0;
		count++;
	} // while end

	printf("\n");
	printf("You Lose ... \n");
	printf("Answer is : ");
	if (strikeCount != 3)
	{
		for (int i = 0; i < MAXSIZE; i++)
		{
			printf("%d ", *(balls + i));
		}
	}
	// 랜덤 숫자 출력
}